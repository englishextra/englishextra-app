...
/*!
 * set click event on data hash links,
 * so that they change window hash
 * @param {Object} [ctx] context HTML Element
 */
var manageDataHashLinks = function (ctx) {
	"use strict";
	ctx = ctx || "";
	var w = window,
	cls = "[data-hash]",
	a = ctx ? BALA.one(cls, ctx) || "" : BALA.one(cls) || "",
	g = function (e) {
		var h = e.dataset.hash || "";
		if (h && parseLink(h).isRelative) {
			/* evento.add(e, "click", changeHash.bind(null, h)); */
			e.onclick = changeHash.bind(null, h);
		}
	};
	if (a) {
		a = ctx ? BALA(cls, ctx) || "" : BALA(cls) || "";
...
var loadInitMasonryGrid = function () {
	ajaxLoadTriggerJS("./cdn/masonry/4.1.1/js/masonry.pkgd.fixed.min.js", initMasonryGrid);
};
evento.add(window, "load", loadInitMasonryGrid);
/*!
...
var initRoutie = function (ctx) {
	...
	if (appContent) {
		routie({
			...
			"/contents": function () {
				loadVirtualPage(ctx, "./pages/contents.html", function () {
